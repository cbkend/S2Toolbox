<#
 .NOTES
    ===========================================================================
        FileName:  S2_API_AddTransactPortal.ps1
    ...
    #region File Cmdlets
        ...
        Param (
		    ...
		    try {
                ...
                $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession
                $session.Cookies.Add((New-Object System.Net.Cookie(".sessionId", "$($Script:sessionID)", "/", "$($Script:urihost)")))
                $object = Invoke-RestMethod -UseBasicParsing -Uri "$($Script:varhtml)://$($Script:urihost)/goform/Dmf/" `
                -Method "POST" `
                -WebSession $session `
                -Headers @{
                  "Origin"="$($Script:varhtml)://$($Script:urihost)"
                  "Referer"="$($Script:varhtml)://$($Script:urihost)/portaldetail.asp?defaultIndex=0&src=&showChrome=1"
                  "Upgrade-Insecure-Requests"="1"
                } `
                ...
                #Write-Output $body
                #Write-Output $PortalObj
                #Write-Output $object
                
                #$object = ConvertFrom-Json -InputObject $object
                if($object.StatusCode -lt 300){
                ...
                Write-Output $PSItem.InvocationInfo.PositionMessage 
                Write-Output "$($Script:varhtml)://$($Script:urihost)/goform/Dmf/`n"
